...
fn registry_loads_and_validates() {
    ...
    assert!(catalog.tools.len() >= 40, "expected at least 40 starter tools");
    assert!(catalog.packs.len() >= 6, "expected starter packs and optional packs");
...
fn workspaces_load_and_have_tmux_minimum() {
    let workspaces = load_workspaces(Path::new("registry/workspaces.yaml")).expect("workspace loads");

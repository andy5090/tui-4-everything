...
fn materialize_spotatui_command() -> String {
    concat!(
        "rust_lld_dir=\"$(dirname \"$(rustc --print target-libdir)\")/bin/gcc-ld\" && ",
        "CARGO_BUILD_JOBS=1 CARGO_HTTP_TIMEOUT=600 CARGO_HTTP_LOW_SPEED_LIMIT=1 CARGO_NET_RETRY=10 ",
        "CARGO_PROFILE_RELEASE_OPT_LEVEL=0 CARGO_PROFILE_RELEASE_LTO=false ",
        "CARGO_PROFILE_RELEASE_CODEGEN_UNITS=16 ",
        "RUSTFLAGS=\"-C link-arg=-B$rust_lld_dir -C link-arg=-fuse-ld=lld\" ",
        "cargo install --locked spotatui --no-default-features --features telemetry,streaming && ",
        "mkdir -p \"$HOME/.local/bin\" && ",
        "printf '%s\\n' ",
        "'#!/bin/sh' ",
        "'config_dir=\"${XDG_CONFIG_HOME:-$HOME/.config}/spotatui\"' ",
        "'profile=\"$config_dir/.t4e-low-resource\"' ",
        "'pending=\"$config_dir/.t4e-low-resource-pending\"' ",
        "'if [ -f \"$profile\" ]; then' ",
        "'  export SPOTATUI_STREAMING_AUDIO_DEVICE=\"${SPOTATUI_STREAMING_AUDIO_DEVICE:-pulse}\"' ",
        "'  if [ -f \"$pending\" ]; then' ",
        "'    mkdir -p \"$config_dir\"' ",
        "'    if [ ! -f \"$config_dir/config.yml\" ]; then' ",
        "'      printf \"behavior:\\n  animation_tick_rate_milliseconds: 100\\n\" > \"$config_dir/config.yml\"' ",
        "'    elif grep -q \"^[[:space:]]*animation_tick_rate_milliseconds:\" \"$config_dir/config.yml\"; then' ",
        "'      sed -i \"s/^[[:space:]]*animation_tick_rate_milliseconds:.*/  animation_tick_rate_milliseconds: 100/\" \"$config_dir/config.yml\"' ",
        "'    elif grep -q \"^behavior:[[:space:]]*$\" \"$config_dir/config.yml\"; then' ",
        "'      sed -i \"/^behavior:[[:space:]]*$/a\\\\  animation_tick_rate_milliseconds: 100\" \"$config_dir/config.yml\"' ",
        "'    else' ",
        "'      printf \"\\nbehavior:\\n  animation_tick_rate_milliseconds: 100\\n\" >> \"$config_dir/config.yml\"' ",
        "'    fi' ",
        "'    if [ -f \"$config_dir/client.yml\" ]; then' ",
        "'      if grep -q \"^streaming_bitrate:\" \"$config_dir/client.yml\"; then' ",
        "'        sed -i \"s/^streaming_bitrate:.*/streaming_bitrate: 160/\" \"$config_dir/client.yml\"' ",
        "'      else' ",
        "'        printf \"\\nstreaming_bitrate: 160\\n\" >> \"$config_dir/client.yml\"' ",
        "'      fi' ",
        "'      rm -f \"$pending\"' ",
        "'    fi' ",
        "'  fi' ",
        "'fi' ",
        "'[ \"${1:-}\" = \"--t4e-configure-only\" ] && exit 0' ",
        "'exec spotatui \"$@\"' ",
        "> \"$HOME/.local/bin/t4e-spotatui\" && ",
        "chmod +x \"$HOME/.local/bin/t4e-spotatui\" && ",
        "config_dir=\"${XDG_CONFIG_HOME:-$HOME/.config}/spotatui\" && ",
        "mkdir -p \"$config_dir\" && ",
        "cpu_model=\"$(sed -n 's/^[Mm]odel name[[:space:]]*:[[:space:]]*//p' /proc/cpuinfo 2>/dev/null | head -n 1)\" && ",
        "cpu_count=\"$(getconf _NPROCESSORS_ONLN 2>/dev/null || printf '1')\" && ",
        "mem_kib=\"$(sed -n 's/^MemTotal:[[:space:]]*\\([0-9][0-9]*\\).*/\\1/p' /proc/meminfo 2>/dev/null)\" && ",
        "low_resource=false && ",
        "case \"${T4E_SPOTATUI_LOW_RESOURCE:-auto}\" in ",
        "1|true|yes) low_resource=true ;; ",
        "0|false|no) low_resource=false ;; ",
        "*) case \"$cpu_model\" in *Pentium*M*|*pentium*m*) low_resource=true ;; esac; ",
        "if [ \"${cpu_count:-1}\" -le 1 ] && [ \"${mem_kib:-0}\" -gt 0 ] && ",
        "[ \"$mem_kib\" -le 1048576 ]; then low_resource=true; fi ;; esac && ",
        "if [ \"$low_resource\" = true ]; then ",
        "touch \"$config_dir/.t4e-low-resource\" \"$config_dir/.t4e-low-resource-pending\" && ",
        "\"$HOME/.local/bin/t4e-spotatui\" --t4e-configure-only; ",
        "else rm -f \"$config_dir/.t4e-low-resource\" \"$config_dir/.t4e-low-resource-pending\"; fi"
    )
    .to_string()
}
